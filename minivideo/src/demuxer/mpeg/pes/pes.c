...
int skip_pes(Bitstream_t *bitstr, PesPacket_t *packet)
{
    ...
    {
        skip_bits(bitstr, (packet->PES_packet_length) * 8);
        TRACE_INFO(MPS, " > skip_pes() >> %i bytes\n", packet->PES_packet_length - 6);
        retcode = SUCCESS;
    }
    else
    {
        TRACE_WARNING(MPS, " > skip_pes() >> do it yourself\n");
    }
...
/*
        TRACE_1(MPS, "  star_offset :: %i\n", packet->packet_start_offset);
        TRACE_1(MPS, "  curr_offset :: %i\n", curr_offset);
        TRACE_1(MPS, "  skip_data() >> %i bytes\n", packet->PES_packet_length);
        TRACE_1(MPS, "  skip_data() >> %i bytes\n", packet->PES_header_data_length);
*/
        TRACE_1(MPS, " > skip_pes_data() >> %i bytes\n", skipsize);
        retcode = SUCCESS;
    }
    else
    {
        TRACE_WARNING(MPS, " > skip_pes_data() >> do it yourself!\n");
    }
