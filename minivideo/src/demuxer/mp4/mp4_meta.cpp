...
static int parse_infe(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
...
int parse_iinf(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
           ...
           bitstream_get_absolute_byte_offset(bitstr) < (box_header->offset_end - 8))
    {
        ...
        if (mp4->run == true && retcode == SUCCESS)
        {
            ...
            {
                case BOX_INFE:
                    retcode = parse_infe(bitstr, &box_subheader, mp4);
                    break;
                default:
                    retcode = parse_unknown_box(bitstr, &box_subheader, mp4->xml);
                    break;
...
int parse_iprp(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
           ...
           bitstream_get_absolute_byte_offset(bitstr) < (box_header->offset_end - 8))
    {
        ...
        if (mp4->run == true && retcode == SUCCESS)
        {
            ...
            {
                case BOX_IPCO:
                    retcode = parse_ipco(bitstr, &box_subheader, mp4);
                    break;
                case BOX_IPMA:
                    retcode = parse_ipma(bitstr, &box_subheader, mp4);
                    break;
                default:
                    retcode = parse_unknown_box(bitstr, &box_subheader, mp4->xml);
                    break;
...
int parse_ipco(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
    ...
    Mp4Track_t *currentTrack = nullptr;
        ...
        if (mp4->run == true && retcode == SUCCESS)
        {
            switch (box_subheader.boxtype)
            {
                case BOX_AVCC: {
                    ...
                    mp4->tracks_count++;
                    currentTrack = mp4->tracks[track_id] = (Mp4Track_t*)calloc(1, sizeof(Mp4Track_t));
                    currentTrack->handlerType = MP4_HANDLER_PICT;
                    currentTrack->codec = CODEC_H264;

                    retcode = parse_avcC(bitstr, &box_subheader, currentTrack, mp4);
                } break;
                case BOX_HVCC: {
                    unsigned int track_id = mp4->tracks_count;
                    mp4->tracks_count++;
                    currentTrack = mp4->tracks[track_id] = (Mp4Track_t*)calloc(1, sizeof(Mp4Track_t));
                    currentTrack->handlerType = MP4_HANDLER_PICT;
                    currentTrack->codec = CODEC_H265;

                    retcode = parse_hvcC(bitstr, &box_subheader, currentTrack, mp4);
                } break;
                case BOX_AV1C: {
                    unsigned int track_id = mp4->tracks_count;
                    mp4->tracks_count++;
...
/*
                case fourcc_be("clap"):
                    retcode = parse_clap(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("pasp"):
                    retcode = parse_pasp(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("colr"):
                    retcode = parse_colr(bitstr, &box_subheader, currentTrack, mp4);
                    break;
*/
                case fourcc_be("ispe"):
                    retcode = parse_ispe(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("pixi"):
                    retcode = parse_pixi(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("irot"):
                    retcode = parse_irot(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("iscl"):
                    retcode = parse_iscl(bitstr, &box_subheader, currentTrack, mp4);
                    break;
                case fourcc_be("rloc"):
                    retcode = parse_rloc(bitstr, &box_subheader, currentTrack, mp4);
                    break;
...
int parse_thmb(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
...
int parse_iref(Bitstream_t *bitstr, Mp4Box_t *box_header, Mp4_t *mp4)
{
           ...
           bitstream_get_absolute_byte_offset(bitstr) < (box_header->offset_end - 8))
    {
        ...
        if (mp4->run == true && retcode == SUCCESS)
        {
            ...
            {
                case fourcc_be("dimg"):
                    retcode = parse_dimg(bitstr, &box_subheader, mp4);
                    break;
                case fourcc_be("thmb"):
                    retcode = parse_thmb(bitstr, &box_subheader, mp4);
                    break;
                case fourcc_be("cdsc"):
                    retcode = parse_cdsc(bitstr, &box_subheader, mp4);
                    break;
                default:
                    retcode = parse_unknown_box(bitstr, &box_subheader, mp4->xml);
                    break;
